...
if [ $# -ne 3 ]; then
 ...
 echo "使用方式：$0 <secretId> <secretKey> <Region>"
...
file="${Region}_cvm_list.csv"
total=`get_cvm_total $secretId $secretKey $Region`
...
if [[ -z  "${total}" ]]; then
  ...
  #Total_ProjectID
  Total_ProjectID=`awk 'NR>2{print line}{line=$0} END {print line}' ${file} | awk -F ',' '{print $3}' | sort | uniq -c`
  #Total_InstanceState
