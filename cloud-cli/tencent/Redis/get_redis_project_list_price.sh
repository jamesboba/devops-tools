...
function get_redis_renew_prepaid_price {
...
echo "Region,ZoneId,ProjectId,InstanceId,InstanceName,Type,ProductType,Engine,Size,RedisReplicasNum,RedisShardNum,RedisShardSize,NoAuth,AutoRenewFlag,BillingMode,Status,Createtime,DeadlineTime,Price" >> ${file}
...
if [[ -z  "${total}" ]]; then
  ...
  while read -r line; do
    ...
    BillingMode=`echo $line | awk -F ',' '{print $15}'`
    InstanceId=`echo $line | awk -F ',' '{print $4}'`
  
    case "$BillingMode" in
      1) 
        price=`get_redis_renew_prepaid_price $secretId $secretKey $Region $InstanceId | jq -r '. | {Price: .Price} | join(",")'`
  ...
  #Total_Price
  Total_Price=`cat ${file} | awk -F ',' '{sum += $19} END {print sum}'`
